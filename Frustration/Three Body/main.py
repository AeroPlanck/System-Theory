...
class ThreeBody(Swarmalators2D):
    def __init__(self, strengthLambda1: float, strengthLambda2: float, 
                 distanceD1: float, distanceD2: float, gamma: float, boundaryLength: float = 10,
    ...
    def _pointTheta(phaseTheta: np.ndarray, other1: np.ndarray, other2: np.ndarray, 
                    omegaTheta: np.ndarray, strengthLambda1: float, strengthLambda2: float, gamma: float,
                    K1: np.ndarray, K2: np.ndarray, dt: float) -> np.ndarray:
        # 计算K2的和
        K2_sum = np.sum(K2, axis=(1, 2))
        
        # 计算三体相互作用项
        three_body_term = strengthLambda2 * np.sum(K2 * (np.sin(other1 + other2 - 2 * phaseTheta + gamma) - np.sin(gamma)), axis=(1, 2))
        
        # 安全的除法操作，避免除零警告
        three_body_normalized = np.divide(three_body_term, K2_sum, out=np.zeros_like(three_body_term), where=(K2_sum != 0))
        
        k1 = (
            omegaTheta 
            #+ strengthLambda1 * np.sum(K1 * np.sin(other1 - phaseTheta), axis=(1, 2))
            + three_body_normalized
        )
        return k1 * dt
    ...
    def __str__(self) -> str:
        ...
        if self.uniform:
            name =  (
                ...
                f"_{self.distanceD1:.2f}_{self.distanceD2:.2f}_{self.randomSeed}_{self.gamma}"
            )
        else:
            name =  (
