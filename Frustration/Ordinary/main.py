...
class SpatialGroups(Swarmalators2D):
    def __init__(self, strengthLambda: float, distanceD0: float, phaseLagAlpha: float,
                 boundaryLength: float = 10, agentsNum: int=1000, dt: float=0.01, 
        ...
        self.phaseLagAlpha = phaseLagAlpha
    ...
    @property
    def pointTheta(self):
        return self._pointTheta(self.phaseTheta, self.omegaTheta, self.strengthLambda, self.phaseLagAlpha, self.dt, self.K)

    @staticmethod
    @nb.njit
    def _pointTheta(phaseTheta: np.ndarray, omegaTheta: np.ndarray, strengthLambda: float, phaseLagAlpha: float,
        ...
        k1 = omegaTheta + strengthLambda * np.sum(K * np.sin(
            adjMatrixTheta - phaseTheta - phaseLagAlpha
        ), axis=0)
        return k1 * h
    ...
    def __str__(self) -> str:
        ...
        if self.uniform:
            name =  f"CorrectCoupling_uniform_{self.strengthLambda:.3f}_{self.distanceD0:.2f}_{self.phaseLagAlpha:.3f}_{self.randomSeed}"
        else:
            name =  f"CorrectCoupling_normal_{self.strengthLambda:.3f}_{self.distanceD0:.2f}_{self.phaseLagAlpha:.3f}_{self.randomSeed}"
        return name
