...
For the common parameters $N=2000$, $D=7d_0$, $K=20.75$, and $v=3$, define
the boundary-lattice constant as the wall arc period
$a_\partial=2\pi R/m$, with $m$ counted separately for each phase-resolved
stream.  At $\alpha=\pi/2$, the particle trajectory gives $m=21$ and
$a_\partial=1.0472d_0$, whereas the right-sided growth-envelope limit gives
$2\pi/k_*^+=1.2333d_0$.  At $\alpha=0.6\pi$, the measured first-shell distance
between bulk vortex centers is $a_{\rm hex}=1.3718d_0$ (three-seed range
$1.3513$--$1.4102d_0$); the regular dispersion maximum $k_*=5.0910/d_0$
predicts $4\pi/(\sqrt{3}k_*)=1.4251d_0$.  At $\alpha=\pi$, both counterpropagating
phase families give $m=16$ and $a_\partial=1.3744d_0$, compared with the
left-sided candidate scale $2\pi/k_*^-=1.2235d_0$.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.74\textwidth]{\detokenize{D:/PythonProject/System Theory/Frustration Induced Lattice/output/Lattice_Scale_Comparison/Representative_Lattice_Scale_Comparison.pdf}}
\caption{Measured and continuum lattice scales.  Error bars at $0.6\pi$
span the three seed means; the boundary mode is identical throughout the
analyzed $\pi/2$ window and for all six seed--stream combinations at $\pi$.
The $\pi/2$ square is the $\alpha\to(\pi/2)^+$ envelope because the exact
critical spectrum has no unique fastest-growing wave number.  The $\pi$
square is the $\alpha\to\pi^-$ envelope because the closed matrix is undefined
at the endpoint.  Thus the regular bulk hexagonal state agrees with the
geometry-corrected spectral scale to $3.7\%$, whereas the two boundary states
retain systematic $12$--$15\%$ offsets: they share the same microscopic
length scale but are not quantitatively selected by the one-sided bulk spectra.}
\label{fig:representative_lattice_scale_comparison}
\end{figure}
