...
For the common parameters $N=2000$, $2R=7d_0$, $K=20.75$, and $v=3$, let
$m$ denote the number of clusters in one phase-resolved boundary stream and
define
\begin{equation}
  a_\partial=\frac{2\pi R_{\rm eff}}{m},
  \qquad
  R_{\rm eff}=\left\langle|\mathbf X_i-\mathbf X_c|\right\rangle_{\rm wall} .
  \label{eq:boundary_lattice_constant}
\end{equation}
The wall average uses the parameter-specific selections stated below.  For a
regular continuum spectrum, we use the notation
\begin{equation}
...
does not define a spectral prediction at the critical point.  Using particles
within $0.25d_0$ of the wall, the three realizations instead give $m=21$ and
$a_\partial/d_0=1.0277$--$1.0286$; the line at $d_0$ in
Fig.~\ref{fig:halfpi_lattice_seed_robustness} is only the microscopic
interaction-distance reference.
...
\caption{Seed-resolved boundary-lattice constant at $\alpha=\pi/2$.
Error bars are temporal standard deviations over iterations
$40000$--$50000$.  The dashed $d_0$ line is a length reference, not a
...
\end{equation}
The three seed means are $1.3513$--$1.4102d_0$, with mean
$1.3718d_0$, or $3.7\%$ below Eq.~\eqref{eq:hex_lattice_spectral_prediction}.
...
\end{equation}
Using the phase-resolved $d_i\le0.50d_0$ shell, counting gives $m=16$ for
both streams in every seed and
$a_\partial=1.3684$--$1.3691d_0$, which is $11.9\%$ above the left-limit
candidate.
...
\caption{Seed- and stream-resolved boundary-lattice constant at
$\alpha=\pi$.  CW and CCW are classified by their phase families.  The
dashed line is the left-limit scale in Eq.~\eqref{eq:pi_left_lattice_scale},
not a spectrum evaluated at $\alpha=\pi$.}
\label{fig:pi_lattice_seed_robustness}
\end{figure}

The seed variations therefore leave the scale comparison unchanged:
$\alpha=\pi/2$ is empirically locked close to $d_0$ without a spectral
prediction, the regular $0.6\pi$ bulk lattice is quantitatively consistent
with the geometry-corrected linear scale, and the $\pi$ boundary lattice is
only of the same microscopic order as its one-sided candidate scale.
