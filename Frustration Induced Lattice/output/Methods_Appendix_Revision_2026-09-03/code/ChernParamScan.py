...
def _make_compute_kwargs(cfg):
    ...
    return {
        ...
        "gap_samples": c.get("gap_samples", 1201),
        "gap_rtol": c.get("gap_rtol", 1e-6),
        "gap_atol": c.get("gap_atol", 1e-9),
    }
...
def main(config=None, output_dir=None):
    cfg = USER_CONFIG if config is None else config
    output_dir = Path(__file__).resolve().parent / "scan_outputs" if output_dir is None else Path(output_dir)
    ...
    for i_v, v in enumerate(v_values):
        for i_omega, omega in enumerate(omega_values):
            ...
            for i_d0, d0 in enumerate(d0_values):
                ...
                for i_lam, lam in enumerate(lam_values):
                    ...
                    for alpha in alphas:
                        ...
                        for combo in combos:
                            c_val, c_integer, diagnostic = compute_topology(
                                params,
                                target_bands=combo,
                                **compute_kwargs,
                            )
                            ...
                            status_rows.append({
                                "v": v, "omega": omega, "lambda": lam, "d0": d0,
                                "rho0": rho0, "alpha_over_pi": alpha / np.pi,
                                "bands": "+".join(map(str, combo)),
                                "C_raw": c_val if np.isfinite(c_val) else "NA",
                                "C_integer": c_integer if c_integer is not None else "NA",
                                "valid": diagnostic["valid"], "status": diagnostic["status"],
                                "reason": diagnostic["reason"],
                                "min_relative_gap": diagnostic["min_relative_gap"],
                                "validation_scope": diagnostic["validation_scope"],
                            })
                        group_done += 1
                        global_done += 1
            ...
            status_path = output_dir / f"status_v_{tag_v}_omega_{tag_omega}.csv"
