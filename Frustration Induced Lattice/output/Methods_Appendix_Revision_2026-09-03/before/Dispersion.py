...
def M_matrix_vectorized(kx, ky, v, omega, lam, alpha, rho0, d0):
    ...
    # D0 denominator
    D0_val = 2.0 * omega - 2.0 * lam * np.sin(alpha) * G0 * rho0

    # Prepare components
    zeros = np.zeros_like(kx, dtype=np.complex128)
    ...
    diag = (lam * rho0 / 2.0) * G_q * np.cos(alpha)
    m11 = diag
    m22 = diag
    
    term12 = -omega + lam * rho0 * G0 * np.sin(alpha) - \
             (lam * rho0 / 2.0) * G_q * np.sin(alpha) + \
             (v**2 / (4.0 * D0_val)) * q_sq
