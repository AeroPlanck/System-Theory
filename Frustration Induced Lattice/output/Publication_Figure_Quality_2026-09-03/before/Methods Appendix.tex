...
selected by Eq.~\eqref{eq:high_alpha_candidate_filter}, not the net chirality
...
\end{equation}
The horizontal levels $\operatorname{Im}\sigma=\pm10$ are retained as
empirically chosen observation references. At $\alpha=\pi/2$, the matched
bulk has spectrum $0,\pm i\sqrt{b(k)^2+v^2k^2/2}$, with minimum nonzero
frequency $K/2=10.375$, so both references are bulk line gaps.
At the other four sampled phase lags they intersect the bulk spectrum and
are only observation references. Even in the threshold case, finite-strip
counts alone do not establish all convergence assumptions of the index relation.
The corresponding $N_y=101$ calculation provides a momentum-step comparison,
not a joint UV or infinite-width convergence claim. For both grids, all five
phase lags give $(\mathrm{SF}_{L,\ell},\mathrm{SF}_{R,\ell})=(2,-2)$ at each
of the two references, using the stated orientation and persistent-edge rule.
The machine-readable
output retains parameters, resolved and excluded crossings, and the detected
bulk intersections for every phase lag.

\begin{figure}[htbp]
 \centering
 \includegraphics[width=\textwidth]{Figures/strip_circle_matched_alpha_sweep.pdf}
