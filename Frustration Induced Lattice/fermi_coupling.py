...
class FermiCouplingPhaseLagPatternFormation(Swarmalators2D):
    
    def __init__(self, strengthK: float, distanceR0: float, fermiBeta: float, phaseLagA0: float,
        ...
        self.distanceR0 = distanceR0  # 费米分布的特征距离
        self.fermiBeta = fermiBeta    # 费米分布的陡峭程度参数
    ...
    def dotPhase(self) -> np.ndarray:
        return self._calc_dot_phase(self.deltaTheta, self.fermiCoupling, self.freqOmega, 
                                    self.strengthK, self.phaseLagA0, self.agentsNum)
    ...
    @property
    def fermiCoupling(self) -> np.ndarray:
        distances = self.distance_x(self.deltaX)
        return self._fermi_distribution(distances, self.distanceR0, self.fermiBeta)
    
    @staticmethod
    @nb.njit
    def _fermi_distribution(distances: np.ndarray, r0: float, beta: float) -> np.ndarray:
        return 1.0 / (1.0 + np.exp((beta/r0) * (distances - r0)))

    @staticmethod
    @nb.njit
    def _calc_dot_phase(deltaTheta: np.ndarray, coupling: np.ndarray, omega: np.ndarray, 
                        K: float, phaseLagA0: float, agentsNum: int) -> np.ndarray:
        coupling_sum = np.zeros(deltaTheta.shape[0])
        for idx in range(deltaTheta.shape[0]):
            weighted_coupling = 0.0
            total_weight = 0.0
            for j in range(deltaTheta.shape[1]):
                if idx != j:  # 排除自身
                    weight = coupling[idx, j]
                    weighted_coupling += weight * (np.sin(deltaTheta[idx, j] + phaseLagA0) - np.sin(phaseLagA0))
                    total_weight = agentsNum
            
            """ if total_weight > 0:
                coupling_sum[idx] = weighted_coupling / total_weight
            else:
                coupling_sum[idx] = 0.0 """
            coupling_sum[idx] = weighted_coupling / total_weight   

        return K * coupling_sum + omega
    ...
    def __str__(self):
        return (
            f"FermiCoupling_K{self.strengthK:.1f}_R{self.distanceR0:.1f}_"
            f"B{self.fermiBeta:.0f}_A{self.phaseLagA0:.2f}_N{self.agentsNum}_"
            f"seed{self.randomSeed}"
        )


class FermiCouplingNoCounter(FermiCouplingPhaseLagPatternFormation):
    """费米耦合无反向项版本"""
    ...
    def _calc_dot_phase(deltaTheta: np.ndarray, coupling: np.ndarray, omega: np.ndarray, 
        ...
        for idx in range(deltaTheta.shape[0]):
            ...
            for j in range(deltaTheta.shape[1]):
                if idx != j:
                    weight = coupling[idx, j]
                    weighted_coupling += weight * np.sin(deltaTheta[idx, j] + phaseLagA0)
                    total_weight += weight
...
class FermiCouplingAdditive(FermiCouplingPhaseLagPatternFormation):
    """费米耦合加性版本"""
    ...
    def _calc_dot_phase(deltaTheta: np.ndarray, coupling: np.ndarray, omega: np.ndarray, 
        ...
        for idx in range(deltaTheta.shape[0]):
            ...
            coupling_sum[idx] = weighted_coupling
                
        return K * coupling_sum + omega


class FermiCouplingOnlyCounter(FermiCouplingPhaseLagPatternFormation):
    """费米耦合仅反向项版本"""
    ...
    def _calc_dot_phase(deltaTheta: np.ndarray, coupling: np.ndarray, omega: np.ndarray, 
        ...
        for idx in range(deltaTheta.shape[0]):
            ...
            for j in range(deltaTheta.shape[1]):
                if idx != j:
                    weight = coupling[idx, j]
                    weighted_coupling += weight * (np.sin(deltaTheta[idx, j]) - np.sin(phaseLagA0))
                    total_weight += weight
...
class FermiCouplingPurePhaseFrustration(FermiCouplingPhaseLagPatternFormation):
    """费米耦合纯相位挫折版本"""
    
    def __init__(self, strengthK: float, fermiBeta: float, phaseLagA0: float, 
                 freqDist: str = "uniform", initPhaseTheta: np.ndarray = None, 
                 omegaMin: float = 0.1, deltaOmega: float = 1, 
                 agentsNum: int = 1000, dt: float = 0.01, 
                 tqdm: bool = False, savePath: str = None, shotsnaps: int = 10, 
                 randomSeed: int = 10, overWrite: bool = False):
        super().__init__(strengthK, 0, fermiBeta, phaseLagA0, 0, 0, 
...
class FermiCouplingPhaseLagPatternFormation1D(FermiCouplingPhaseLagPatternFormation):
...
class StateAnalysis:
    def __init__(self, model: FermiCouplingPhaseLagPatternFormation = None):
        if model is None:
            return
        self.model = model
        ...
        self.TNum = TNum
        if isinstance(model, FermiCouplingPhaseLagPatternFormation1D):
    ...
    def plot_spatial(self, ax: plt.Axes = None, 
        ...
        if isinstance(self.model, FermiCouplingPhaseLagPatternFormation1D):
