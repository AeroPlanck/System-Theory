"""Generate two additional 50000-step alpha=0.6*pi circular trajectories.
...
DATA_DIR = ROOT / "data" / "alpha06_bulk_N2000_steps50000_snap50"
N = 2000
STEPS = 50_000
SNAP = 50
SEEDS = (1, 17)
...
def run(seed: int) -> None:
    ...
    if destination.exists():
        ...
        print(f"seed={seed}: reused {frames} frames, {size/2**20:.1f} MiB", flush=True)
        return
    ...
    try:
        ...
        print(f"seed={seed}: generated {frames} frames, {size/2**20:.1f} MiB", flush=True)
    finally:
        if temporary.exists():
            temporary.unlink()
