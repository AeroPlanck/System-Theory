...
class ChiralSolvable2DMul(ChiralSolvable2D):
    @staticmethod
    @nb.njit
    def _G(deltaX: np.ndarray) -> np.ndarray:
        return np.cos(deltaX[:, :, 0]) * np.cos(deltaX[:, :, 1])
...
class ChiralSolvable2DMul2V(ChiralSolvable2DMul):
    # @staticmethod
    # @nb.njit
    # def _G(deltaX: np.ndarray) -> np.ndarray:
    #     return np.cos(deltaX[:, :, 0]) * np.cos(deltaX[:, :, 1])
    ...
    @nb.njit
    def _velocity(speedV: float, phaseTheta: np.ndarray) -> np.ndarray:
        return speedV * np.cos(2 * phaseTheta), speedV * np.sin(2 * phaseTheta)

    def __str__(self):
        name = (
            f"ChiralSolvable2DMul2V_"
            f"Agents.{self.agentsNum}_"
            f"K.{self.K}_"
            f"dt.{self.dt}_"
            f"distribution.{self.distribution}_"
